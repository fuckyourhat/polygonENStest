...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld) payable ERC721("420 Name Service", "420") {
    ...
    function register(string calldata name) public payable {
        ...
        // Combine name passed to function with tld
		...
		// Create the JSON metadata as base64
        string memory json = Base64.encode(
            bytes(
                string(
                    ...
                    '{"name": "',
                    _name,
                    '", "description": "A domain on the 420 name service", "image": "data:image/svg+xml;base64,',
        ...
        // Keep a record of the domain registered so we can output it later.
    ...
    //make sure the domain is between three and ten characters
    function valid(string calldata name) public pure returns(bool) {
        return StringUtils.strlen(name) >= 3 && StringUtils.strlen(name) <= 10;
    }

    // Gets list of all domains on the contract
